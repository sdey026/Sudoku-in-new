...
## Overview
Welcome to the Grid Game project! This game uses a two-dimensional grid to create an engaging and interactive Sudoku experience. The game is implemented in C and uses the ncurses library for terminal-based graphics. The primary goal is to fill the grid following Sudoku rules, with built-in hints and input validation.
